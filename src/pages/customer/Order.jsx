...
const Order = () => {
  ...
  const handleDecrement = (menu) => {
    ...
    const existingItemIndex = updatedCart.findIndex((item) => item.kode === menu.kode);
    ...
    if (existingItemIndex !== -1) {
      if (updatedCart[existingItemIndex].jumlah > 1) {
        ...
        if (quantityElement) {
          quantityElement.innerText = updatedCart[existingItemIndex].jumlah.toString();
        }
      } else {
  ...
  return (
    <div className='container mt-3'>
      <input type='text' placeholder='Cari menu...' className='form-control mb-5' onChange={(e) => setSearchTerm(e.target.value)} />
      <div className='row'>
        {searchResults.map((menu, index) => (
          <div className='col-lg-6 col-xl-4 mb-4' key={menu.kode}>
            <div className='card' style={cardStyle}>
              <div className='card-body'>
                ...
                <div className='d-flex justify-content-center align-items-center mt-3'>
                  ...
                  <div className='mx-3'>
                    <b style={{ fontSize: '30px' }} id={'jumlah_' + menu.kode}>
                      {cart.find((item) => item.kode === menu.kode)?.jumlah || 0}
                    </b>
                  </div>
      ...
      {showButton && (
        <div className='row fixed-bottom'>
          <div className='col-12'>
            {cart.length === 0 ? (
              <button className='btn btn-secondary form-control' disabled style={{ height: '50px' }}>
                Checkout Sekarang ({cart.length})
              </button>
            ) : (
              <button className='btn btn-primary form-control'
                style={{ height: '50px' }} onClick={() => handlerData()}>Checkout Sekarang ({cart.length})</button>
            )}
          </div>
        </div>
      )}
